...
ExportPredicate isWithin(String libraryRootName) {
  return (element) => element.librarySource!.fullName.startsWith('/$libraryRootName/');
}
...
class _LibraryWithVisibility {
  ...
  bool isRelevant(LibraryPredicate? whereLibrary) => whereLibrary?.call(library) ?? true;

  Iterable<Element> get exportedTopLevelElements => library.topLevelElements.where(_isExported);
  ...
  bool operator ==(Object other) =>
      ...
      other is _LibraryWithVisibility && runtimeType == other.runtimeType && library == other.library;
...
extension on Iterable<NamespaceCombinator> {
  Set<String> get hidden => whereType<HideElementCombinator>().expand((e) => e.hiddenNames).toSet();

  Set<String> get shown => whereType<ShowElementCombinator>().expand((e) => e.shownNames).toSet();
}
